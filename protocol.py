...
metadata = {
    ...
    'author': 'OpentronsAI',
    'description': 'Calibration protocol using gradient descent to optimize liquid handling parameters',
    'source': 'OpentronsAI'
}
...
def run(protocol: protocol_api.ProtocolContext):
    ...
    # Load pipettes
    pipette_8ch_1000 = protocol.load_instrument('flex_8channel_1000', 'left', tip_racks=[tiprack_1000])
    pipette_8ch_50 = protocol.load_instrument('flex_8channel_50', PIPETTE_MOUNT, tip_racks=[tiprack_50])
    ...
    horizontal_sweep_points = [(0, 0), (1, 0), (-1, 0), (0, 1), (0, -1)]  # relative positions
    ...
    def evaluate_liquid_height(well, pipette, expected_height):
        ...
        for x_offset, y_offset in horizontal_sweep_points:
            try:
                ...
                target_location = well.bottom(expected_height).move(protocol.geometry.Point(x_offset, y_offset, 0))
    ...
    def evaluate_bubblicity(well, pipette, expected_height):
        ...
        for height_increment in bubble_check_increments:
            ...
            for x_offset, y_offset in horizontal_sweep_points:
                try:
                    target_location = well.bottom(check_height).move(protocol.geometry.Point(x_offset, y_offset, 0))
                    pipette.move_to(target_location)
    ...
    for col_idx in range(num_columns):
        ...
        if col_idx == 0:
            ...
            if well_data:
                ...
                adjustment_factor = calculate_gradient_adjustment(previous_score, last_result['bubblicity_score'])
                ...
                current_params['aspiration_withdrawal_rate'] += adjustment_factor * gradient_step['aspiration_withdrawal_rate']
        ...
        protocol.comment(f"Well {well}: Height OK: {height_status}, Bubblicity: {bubblicity_score:.2f}")
    ...
    if well_data:
        ...
        if successful_results:
            ...
            protocol.comment(f"Optimal bubblicity score: {optimal_result['bubblicity_score']:.2f}")
