...
CSV_PATH = os.path.join(os.path.dirname(__file__), "liquid_classes.csv")
...
def add_liquid_class_interactive():
    ...
    try:
        ...
        if 0 <= liquid_choice < len(built_in_liquids):
            ...
            # This is a custom liquid selection
            custom_index = liquid_choice - len(built_in_liquids)
            custom_liquid_name = sorted(custom_liquids)[custom_index]
            print(f"Selected existing custom liquid: {custom_liquid_name}")
            liquid_type = CustomLiquid(custom_liquid_name)
        else:
            print("Invalid liquid selection number")
            return
    except ValueError:
        # If not a number, treat as custom liquid name
        custom_liquid_name = liquid_input
        if not custom_liquid_name:
            print("Liquid name cannot be empty")
            return

        # Check if this custom liquid already exists
        if custom_liquid_name in custom_liquids:
            print(f"Using existing custom liquid: {custom_liquid_name}")
            liquid_type = CustomLiquid(custom_liquid_name)
        else:
            # This is a new custom liquid
            print(f"Using new custom liquid type: {custom_liquid_name}")
            print("Note: Custom liquids will be stored but may not work with all features.")
            liquid_type = CustomLiquid(custom_liquid_name)
    ...
    # For custom liquids, we need to handle them differently
    if hasattr(liquid_type, "name") and not isinstance(liquid_type, LiquidType):
        # This is a custom liquid - use LiquidClassParams with custom liquid object
        ...
        key = f"{pipette_type.value}_{custom_liquid_name}"
        liquid_class_registry._liquid_classes[key] = new_params
        auto_save_registry()
...
def show_available_options():
    """Display all available pipettes and liquids"""
    ...
    if custom_liquids:
        print("\nCustom Liquids:")
        print("-" * 20)
        for i, liquid in enumerate(sorted(custom_liquids), 1):
            print(f"  {i}. {liquid}")

    print("\nParameter Descriptions:")
    print("-" * 30)
    print("  Aspiration Rate: Speed of liquid uptake (µL/s)")
    print("  Aspiration Delay: Time to wait after aspiration (s)")
    print("  Aspiration Withdrawal Rate: Speed of pipette withdrawal (mm/s)")
    print("  Dispense Rate: Speed of liquid dispensing (µL/s)")
    print("  Dispense Delay: Time to wait after dispensing (s)")
    print("  Blowout Rate: Speed of air blowout (µL/s)")
    print("  Touch Tip: Whether to touch tip after dispensing (Yes/No)")
