...
metadata = {
    "protocolName": "Liquid Class Calibration with Gradient Descent - Single Channel",
    "author": "Roman Gurovich",
    "description": (
        "Single-channel calibration protocol using gradient descent to optimize liquid handling parameters across all 96 wells"  # noqa: E501
    ),
...
def run(protocol: protocol_api.ProtocolContext):
    ...
    # Load pipettes - using 8-channel but targeting individual wells
    pipette_1000 = protocol.load_instrument("flex_8channel_1000", "left", tip_racks=[tiprack_1000])
    pipette_50 = protocol.load_instrument("flex_8channel_50", PIPETTE_MOUNT, tip_racks=[tiprack_50])
    ...
    for well_idx, well in enumerate(test_wells):
        ...
        height_status = evaluate_liquid_height(well, pipette_50, expected_liquid_height)

        # Step 1.4: Evaluate bubblicity targeting individual well
        if not height_status:
            ...
            bubblicity_score = evaluate_bubblicity(well, pipette_50, expected_liquid_height)
    ...
    protocol.comment("Single-channel liquid class calibration protocol completed")
